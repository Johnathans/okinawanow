...
export default function ContactPreferences({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.preferredContactTimes.length === 0) {
            ...
            <div className="d-flex flex-wrap gap-2">
              {(['email', 'phone', 'line', 'whatsapp'] as const).map((method) => (
                <button
                  ...
                  onClick={() => {
                    const newMethods = formData.preferredContactMethods.includes(method)
                      ? formData.preferredContactMethods.filter((m) => m !== method)
                      : [...formData.preferredContactMethods, method];
            ...
            <div className="d-flex flex-wrap gap-2">
              {(['morning', 'afternoon', 'evening'] as const).map((time) => (
                <button
                  ...
                  onClick={() => {
                    const newTimes = formData.preferredContactTimes.includes(time)
                      ? formData.preferredContactTimes.filter((t) => t !== time)
                      : [...formData.preferredContactTimes, time];
